...
ShootBallCommandGroup::ShootBallCommandGroup(ShooterSubsystemPID* shootersubsystemPID, ConveyorSubsystem* conveyorsubsystem, OrganizerSubsystem* organizersubsystem, HoodSubsystem* hoodsubsystem, TurretSubsystem* turretsubsystem, int numberOfBalls, int velocity, bool usingVision) {
  l_numberofballs = numberOfBalls;
  l_velocity = velocity;
  ...
  AddCommands(
    ...
    GetToVelocityCommand(shootersubsystemPID, hoodsubsystem, turretsubsystem, l_velocity, l_usingvision), //was false but it should be true to adjust hood and flywheel speed
     ...
     LightCurtainCounterCommand(shootersubsystemPID, conveyorsubsystem, organizersubsystem, turretsubsystem, hoodsubsystem, l_numberofballs),
    //  TurnTurretSpecificAngleCommand(turretsubsystem, 0),
     LimelightOn(turretsubsystem, false),
     IntakeSolOutCommand(conveyorsubsystem),WaitTimeCommand(0),
     ShootertoZero(hoodsubsystem, shootersubsystemPID)
     );
}
